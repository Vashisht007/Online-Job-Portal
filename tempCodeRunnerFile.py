...
def login():
    ...
    if request.method=="POST":
        ...
        # password2=check_password_hash(user.password,password)
        # print(password2)
        ...
        if user.email and check_password_hash(user.password,password):
            ...
            session['user']=None
            if det:
                dict = {"demail":det.demail,"firstName":det.firstName,"lastName":det.lastName,"ddob":det.ddob,"lastjobdesignation":det.lastjobdesignation,"lastcompany":det.lastcompany,"domain":det.domain,"phoneno":det.phoneno,"address":det.address,"salary":det.salary}
                session['user'] = dict
            return render_template("userindex.html")
